...
class AttnTextCls(FeatureAttrMethod):
    def __init__(self, model, # img_dim=224, num_patches=14, patch_size=16, 
                 attn_kwargs={'output_attentions': True, 'return_dict': True}):
        super().__init__(model)
        self.attn_kwargs = attn_kwargs
    ...
    def forward(self, x, labels=None, kwargs={}, return_groups=False):
        ...
        with torch.no_grad():
            original_outputs = self.model(x, **self.attn_kwargs, **kwargs)
        original_logits = original_outputs.logits
        # print('original_outputs.attentions[-1]', original_outputs.attentions[-1].shape)
        attn = original_outputs.attentions[-1].mean(dim=1)[:,0]
